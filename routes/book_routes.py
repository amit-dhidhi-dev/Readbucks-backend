...
class BookService:
    ...
    @staticmethod
    async def get_books(
        skip: int = 0, limit: int = 20, filters: Optional[dict] = None
    ) -> List[BookInDB]:
        query = filters or {}
    ...
    async def update_book(book_id: str, update_data: BookUpdate) -> BookInDB:
        ...
        await books_collection.update_one(
            {"_id": ObjectId(book_id)}, {"$set": update_dict}
        )
    ...
    async def search_books(search_request: BookSearchRequest) -> dict[str, any]:
        ...
        cursor.skip((search_request.page - 1) * search_request.limit).limit(
    ...
    async def add_review_to_book(book_id: str, review_data: Review) -> BookInDB:
        ...
        new_average_rating = (
            (book.get("average_rating", 0) * book.get("total_ratings", 0))
            + review_data.rating
        ) / new_total_ratings
...
# Helper functions
def book_in_db_to_response(
    book: BookInDB, include_content: bool = False
) -> BookResponse:
    base_data = {
        "id": str(book.id),
        "title": book.title,
    ...
    # Handle book_content_url conversion
    book_content_data = None
    if include_content and book.book_content_url:
        if isinstance(book.book_content_url, str):
            # If it's a single string, determine type and create BookConentType
            if book.book_content_url.endswith(".pdf"):
                book_content_data = BookConentType(pdf=book.book_content_url)
            elif book.book_content_url.endswith(".docx"):
                book_content_data = BookConentType(docx=book.book_content_url)
            elif book.book_content_url.endswith(".epub"):
                book_content_data = BookConentType(epub=book.book_content_url)
            else:
                # Default to pdf if extension not clear
                book_content_data = BookConentType(pdf=book.book_content_url)
        elif isinstance(book.book_content_url, dict):
            # If it's already a dictionary, convert to BookConentType
            book_content_data = BookConentType(**book.book_content_url)
        elif (
            hasattr(book.book_content_url, "pdf")
            or hasattr(book.book_content_url, "docx")
            or hasattr(book.book_content_url, "epub")
        ):
            # If it's already a BookConentType-like object
            book_content_data = book.book_content_url
    book.book_content_url = book_content_data
    if include_content:
        return BookDetailResponse(
            **base_data,
            ...
            isbn=book.isbn,
            tags=book.tags,
            book_content_url=book.book_content_url if include_content else None,
            sample_chapter_url=(
                str(book.sample_chapter_url) if book.sample_chapter_url else None
            ),
...
# FastAPI Routes
@router.post(
    "/books-create/", response_model=BookResponse, status_code=status.HTTP_201_CREATED
)
...
async def get_book(book_id: str, Authorization: str = Header(str)):
    ...
    has_access = (
        book.is_free
        or book.access_level == BookAccessLevel.FREE
        or check_user_book_access(user["user_id"], book_id)
    )
...
async def update_book(
    book_id: str, update_data: BookUpdate, Authorization: str = Header(str)
):
...
async def add_quiz_to_book(
    book_id: str, quiz_data: Quiz, user: Dict = Depends(get_current_user)
):
...
async def add_review(
    book_id: str, review_data: Review, user: Dict = Depends(get_current_user)
):
...
async def delete_book(book_id: str, Authorization: str = Header(None)):
    ...
    book = await books_collection.find_one_and_delete(
        {"created_by": data["user_id"], "_id": ObjectId(book_id)}
    )
...
@router.get("/get-user-book")
async def get_user_book(Authorization: str = Header(None)):
    if not Authorization:
        raise HTTPException(status_code=400, detail="Authorization header missing")
    ...
    user_books = books_collection.find({"created_by": data["user_id"]})
    # print('user_books ->',user_books)
    books_list = []
    async for book in user_books:
        book["_id"] = str(
            book["_id"]
    ...
    return {"user_id": data["user_id"], "books": books_list}
